...
const STAKING_ADDRESS = "0xD039050D7Cd7B0974eD50358DafC99eDEF89a98D";
...
function App() {
  ...
  const [isClaimSecret, setIsClaimSecret] = useState(false);
  ...
  const commitRewardClaim = async () => {
    try {
       if (hasCommitment) {
        toast.error("Finish claiming first!");
        return;
    }
      setIsClaimSecret(true)
      ...
      const hash = ethers.keccak256(
        ethers.AbiCoder.defaultAbiCoder().encode(
          ["uint256", "string"],
          [rewards.toString(), secret]
        )
      );
      const commitTx = await stakingContract.commitRewardClaim(hash);
      await commitTx.wait();
      setHasCommitment(true);
      toast.success("Commit successful! Proceed to claim");
      setIsClaimSecret(false)
    } catch (e) {
  ...
  const claimRewards = async () => {
    try {
      ...
      if (!stakingContract || !claimSecret) return;
      const rewards = await stakingContract.getPendingRewards(account);
      const tx = await stakingContract.claimRewards(rewards.toString(),
      claimSecret);
      await tx.wait();
        ...
        <div className="bg-slate-700 h-screen grid grid-cols-12">
          ...
          <div className="col-span-6 flex flex-col justify-center items-center ">
            ...
            <div className="flex flex-col justify-center items-center">
              ...
              <button type="button" onClick={claimRewards} title={!claimSecret? "click on prepare claim" : ""}  disabled={!claimSecret || isClaimLoading} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4  font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 ml-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                {isClaimLoading ? (
              ...
              <button type="button" onClick={commitRewardClaim} disabled={isClaimSecret} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4  font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 ml-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                {isClaimSecret ? (
                  <Loader text="committing.."></Loader>
                ) : (
                  "prepare claim"
                )}
              </button>
